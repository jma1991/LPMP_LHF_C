...
sce <- slingshot(sce, clusterLabels = colLabels(sce), reducedDim = "PCA", start.clus = 3)
```
...
```{r}
cur.2 <- slingCurves(cur)[[2]]

cur.2 <- data.frame(cur.2$s[cur.2$ord, ])

colnames(cur.2) <- c("Dim.1", "Dim.2")
```
...
ggplot(dat, aes(PCA.1, PCA.2)) + 
  geom_point(aes(colour = label)) + 
  geom_path(data = cur.2, aes(Dim.1, Dim.2), arrow = arrow()) + 
  scale_colour_tableau() + 
  labs(x = "PCA 1", y = "PCA 2", colour = "Label") + 
  coord_fixed() + 
  theme_bw()
...
```{r}
ggplot(dat, aes(PCA.1, PCA.2)) + 
  geom_point(aes(colour = slingPseudotime_2), size = 2) + 
  geom_path(data = cur.2, aes(Dim.1, Dim.2), arrow = arrow()) + 
  scale_colour_viridis_c() + 
  labs(x = "PCA 1", y = "PCA 2", colour = "Pseudotime") + 
  coord_fixed() + 
...
res <- TSCAN::testPseudotime(sce, pseudotime = sce$slingPseudotime_2)
```
...
```{r fig.width = 12, fig.height = 16}
plotHeatmap(
  object = sce[, !is.na(sce$slingPseudotime_2)],
  features = rownames(sig)[ind],
  exprs_values = "logcounts",
  center = TRUE,
  zlim = c(-4, +4),
...
```{r}
cur.3 <- slingCurves(cur)[[3]]

cur.3 <- data.frame(cur.3$s[cur.3$ord, ])

colnames(cur.3) <- c("Dim.1", "Dim.2")
```
...
ggplot(dat, aes(PCA.1, PCA.2)) + 
  geom_point(aes(colour = label)) + 
  geom_path(data = cur.3, aes(Dim.1, Dim.2), arrow = arrow()) + 
  scale_colour_tableau() + 
  labs(x = "PCA 1", y = "PCA 2", colour = "Label") + 
  coord_fixed() + 
  theme_bw()
...
```{r}
ggplot(dat, aes(PCA.1, PCA.2)) + 
  geom_point(aes(colour = slingPseudotime_3), size = 2) + 
  geom_path(data = cur.3, aes(Dim.1, Dim.2), arrow = arrow()) + 
  scale_colour_viridis_c() + 
  labs(x = "PCA 1", y = "PCA 2", colour = "Pseudotime") + 
  coord_fixed() + 
...
res <- TSCAN::testPseudotime(sce, pseudotime = sce$slingPseudotime_3)
```
...
```{r fig.width = 12, fig.height = 16}
plotHeatmap(
  object = sce[, !is.na(sce$slingPseudotime_3)],
  features = rownames(sig)[ind],
  exprs_values = "logcounts",
  center = TRUE,
  zlim = c(-4, +4),
  symmetric = TRUE,
  colour_columns_by = "label",
  order_columns_by = "slingPseudotime_3",
)
```
