...
knitr::opts_chunk$set(
  autodep    = TRUE,
  cache      = TRUE,
  cache.path = "cache/13-atlas-clustering.Rmd/",
  dev        = "png",
  error      = FALSE,
  message    = FALSE,
  warning    = FALSE
...
```{r}
sce <- readRDS("output/12-atlas-trajectory.rds")
```
...
```{r}
var <- c("cluster_walktrap", "celltype", "stage", "source")
```

#### MNN

```{r, fig.height = 8, fig.width = 10}
plt <- lapply(var, function(x) plotReducedDim(sce, "corrected", colour_by = x) + coord_fixed())
...
```{r}
var <- c("cluster_louvain", "celltype", "stage", "source")
```
...
```{r}
var <- c("cluster_kmeans", "celltype", "stage", "source")
```
...
```{r}
var <- c("cluster_hclust", "celltype", "stage", "source")
```
...
```{r}
saveRDS(sce, file = "output/13-atlas-clustering.rds")
```
